...
public:
    ...
    VOID    SetName( CHAR* strName )     { strcpy( m_strName, strName ); }
    VOID    SetMatrix( D3DMATRIX* pmat ) { m_mat = *pmat; }
    VOID    SetNormals( D3DVECTOR* pNormals );
    VOID    SetTextureCoords( FLOAT* pTexCoords );
    VOID    SetMaterialData( DWORD dwMaterial, D3DMATERIAL7* pmtrl, CHAR*strName );
...
public:
    ...
    CD3DFileObject* FindObject( CHAR* strName );
