...
{
    x = sin( fTheta/2.0f ) * v.x;
    y = sin( fTheta/2.0f ) * v.y;
    z = sin( fTheta/2.0f ) * v.z;
    w = cos( fTheta/2.0f );
}
...
{
    fTheta = acos(w) * 2.0f;
    v.x    = x / sin( fTheta/2.0f );
    v.y    = y / sin( fTheta/2.0f );
    v.z    = z / sin( fTheta/2.0f );
}
...
{
    FLOAT fSinYaw   = sin( fYaw/2.0f );
    FLOAT fSinPitch = sin( fPitch/2.0f );
    FLOAT fSinRoll  = sin( fRoll/2.0f );
    FLOAT fCosYaw   = cos( fYaw/2.0f );
    FLOAT fCosPitch = cos( fPitch/2.0f );
    FLOAT fCosRoll  = cos( fRoll/2.0f );
                                   ...
                                   D3DMATRIX& mat )
{
    ...
    {
        FLOAT s = sqrt( mat._11 + mat._22 + mat._33 + mat._44 );
                              ...
                              FLOAT fAlpha )
{
    ...
    if( fCosTheta + 1.0f > 0.05f )
    {
        ...
        else // Otherwise, do spherical interpolation
        {
            ...
            fScale1 = sin( fTheta * (1.0f-fAlpha) ) / fSinTheta;
            fScale2 = sin( fTheta * fAlpha ) / fSinTheta;
        }
    }
    else
    {
        ...
        Bz = -Aw;
        Bw =  Az;
        fScale1 = sin( g_PI * (0.5f - fAlpha) );
        fScale2 = sin( g_PI * fAlpha );
    }
