...
//-----------------------------------------------------------------------------
static TCHAR  g_strTexturePath[512] = TEXT(""); // Path for files
...
HRESULT TextureContainer::LoadImageData()
{
    ...
    strcpy( strPathname, g_strTexturePath );
    strcat( strPathname, m_strName );
    if( NULL == ( file = fopen( strPathname, "rb" ) ) )
    {
        // Then check if the file exists in the DirectX SDK media path
        strcpy( strPathname, D3DUtil_GetDXSDKMediaPath() );
        strcat( strPathname, m_strName );
        if( NULL == ( file = fopen( strPathname, "rb" ) ) )
    ...
    if( NULL == ( strExtension = _tcsrchr( m_strName, TEXT('.') ) ) )
...
HRESULT TextureContainer::Restore( LPDIRECT3DDEVICE7 pd3dDevice )
{
    ...
    // Adjust width and height, if the device requires it
...
VOID D3DTextr_SetTexturePath( TCHAR* strTexturePath )
{
    if( NULL == strTexturePath )
        strTexturePath = TEXT("");
    strcpy( g_strTexturePath, strTexturePath );
}
