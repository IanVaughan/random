...
// Desc: 
//-----------------------------------------------------------------------------
CD3DFileObject::CD3DFileObject( CHAR* strName )
{
    m_pNext        = NULL;
    m_pChild       = NULL;
    m_strName[0]   = 0;
...
VOID CD3DFileObject::Render( LPDIRECT3DDEVICE7 pd3dDevice, BOOL bAlpha )
{
    if( m_bHasMeshData )
    {
        ...
        for( DWORD i=0; i <= m_dwNumMaterials; i++ )
        {
            ...
            {
                CHAR* strTexture   = m_Material[i].m_strTexture;
                            ...
                            (VOID**)&pChildData ) ) )
        {
            ...
            if( TID_D3DRMMeshMaterialList == *pGUID )
            {
                ...
                else
                {
                    ...
                    {
                        for (int iFace=0; iFace<(int)dwNumMatFaces; iFace++)
                        {
                            if ((int)pMatFace[iFace] == iMaterial)
                            {
                                pMesh->AddFace(iMaterial, GetFace(pFaceData, iFace), 1);
                            }
                        }
                    }
                }
    ...
    while( SUCCEEDED( pFileData->GetNextObject( &pChildObj ) ) )
    {
                            ...
                            (VOID**)&pChildData ) ) )
        {
            ...
            if( TID_D3DRMFrameTransformMatrix == *pGUID )
            {
                ...
                {
                    D3DMATRIX matFrame = *(D3DMATRIX*)pData;

                    //
                    // convert from a left-handed cordinate system to
                    // a right-handed one.
                    //
                    matFrame(3,2) *= -1;
                    matFrame(2,1) *= -1;
...
//-----------------------------------------------------------------------------
HRESULT CD3DFile::Load( CHAR* strFilename )
{
    HRESULT                 hr;
    ...
    {
        CHAR strPath[512] = "";
        strcat( strPath, D3DUtil_GetDXSDKMediaPath() );
    ...
    m_pRoot = new CD3DFileObject( "D3DFile_Root" );
...
//-----------------------------------------------------------------------------
HRESULT CD3DFile::GetMeshVertices( CHAR* strName, D3DVERTEX** ppVertices,
...
//       object, and retrieves the vertices for the specified mesh.
//-----------------------------------------------------------------------------
HRESULT CD3DFile::GetMeshIndices( CHAR* strName, WORD** ppIndices,
                                  DWORD* pdwNumIndices )
{
    CD3DFileObject* pObject = FindObject( strName );
	if( pObject )
...
//-----------------------------------------------------------------------------
CD3DFileObject* CD3DFile::FindObject( CHAR* strName )
{
    if( NULL == strName )
        return m_pRoot;
